...
def feat(df):
    ...
    df["tod"] = df["hr"].map(lambda h:"n"if h<=5 else"m"if h<=9 else"d"if h<=15 else"e"if h<=19 else"l")
    ...
    if wcol:
        df["wnd"] = pd.cut(pd.to_numeric(df[wcol], errors="coerce"),
                           [-.1,7,15,25,np.inf],labels=["0","1","2","3"])
    else:
        df["wnd"]="NA"

    df["road"] = df["Street"].fillna(df["Description"]).str.lower().map(
        lambda x:"int" if isinstance(x,str) and re.search(r"\b(i-|interstate|fwy)\b",x)
...
def agg(df,col):
    base = df["sev"].mean()
    g = (df.groupby(col, observed=True)["sev"]
           .agg(["size","mean"])
